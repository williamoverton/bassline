...
resource "null_resource" "build_container" {
  ...
  provisioner "local-exec" {
    command = "docker build -t ${var.app_name}-${random_id.container_id.keepers.new} ${path.module}/src"
  }
...
resource "null_resource" "tag_container" {
  ...
  provisioner "local-exec" {
    command = "docker tag ${var.app_name}-${random_id.container_id.keepers.new} ${aws_ecr_repository.bl_ecr_repo.repository_url}:latest"
  }
...
resource "null_resource" "push_container" {
  ...
  provisioner "local-exec" {
    command = "docker push ${aws_ecr_repository.bl_ecr_repo.repository_url}:latest"
  }
